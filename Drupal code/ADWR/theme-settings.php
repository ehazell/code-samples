...
function adwr_form_system_theme_settings_alter(&$form, &$form_state) {
  ...
  if (!in_array('administrator', $user->roles)) {
    ...
    $form['advanced']['#group'] = '';
    drupal_add_css(drupal_get_path('theme', 'adwr') . '/css/non-admin-theme.css');
  }
  ...
  unset($form['#submit']);
  $form['#submit'][] = 'adwr_settings_form_submit';
  $themes = list_themes();
...
function adwr_settings_form_submit(&$form, $form_state) {
  ...
  if (is_object($image)) {
    if ($image->status == 0) {
      ...
      file_save($image);
      file_usage_add($image, 'adwr', 'theme', 1);
    }
  }
